#!/usr/bin/env bash
# Set up the edge device as the orzo training box — no sudo required.
# Run scripts/setup_swap.sh with sudo FIRST (see that script), then:
# bash scripts/setup_edge device.sh
set -euo pipefail
...
"$VENV_PY" -c "import torch; print('torch', torch.__version__, 'cuda:', torch.cuda.is_available())" || {
 echo "torch CUDA check failed — see train/requirements-edge device.txt for fallbacks"; exit 1; }

echo "== training deps"
uv pip install --python "$VENV_PY" -r train/requirements-edge device.txt
